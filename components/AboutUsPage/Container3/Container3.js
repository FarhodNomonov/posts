import React from 'react'
import Image from 'next/image';

const Container3 = () => {
    return (
        <>


            <div className='flex justify-evenly py-[80px]'>
                <div className='flex flex-col gap-2 items-center'>
                    <div className='relative w-12 h-12'>
                        <Image src="data:image/webp;base64," layout='fill' objectFit='contain' alt="img" />
                    </div>
                    <h1 className='text-[44px] text-black font-semibold'>255</h1>
                    <p className='text-[15px]'>CLIENTS</p>
                </div>
                <div className='flex flex-col gap-2 items-center'>
                    <div className='relative w-12 h-12'>
                        <Image src="data:image/webp;base64," layout='fill' objectFit='contain' alt="img" />
                    </div>
                    <h1 className='text-[44px] text-black font-semibold'>1176</h1>
                    <p className='text-[15px]'>PROJECTS</p>
                </div>
                <div className='flex flex-col gap-2 items-center'>
                    <div className='relative w-12 h-12'>
                        <Image src="data:image/webp;base64," layout='fill' objectFit='contain' alt="img" />
                    </div>
                    <h1 className='text-[44px] text-black font-semibold'>39</h1>
                    <p className='text-[15px]'>COUNTRIES</p>
                </div>
                <div className='flex flex-col gap-2 items-center'>
                    <div className='relative w-12 h-12'>
                        <Image src="data:image/webp;base64," layout='fill' objectFit='contain' alt="img" />
                    </div>
                    <h1 className='text-[44px] text-black font-semibold'>127</h1>
                    <p className='text-[15px]'>TRAVEL PLACES</p>
                </div>
            </div>

            <div className='relative w-full h-[300px] main-div mb-[80px]'>
                <Image src="/images/thai.webp" layout='fill' objectFit='cover' alt="img" />
                <div className='absolute z-10  text-white pl-[145px] h-full flex flex-col justify-center'>
                    <span className='font-semibold text-[40px]'>AYDAR KOL</span>
                    <span>From: <span className='font-bold text-[16px]'>$200</span></span>
                    <button className="text-[14px] font-[700] text-white leading-[53px] px-[46px] relative overflow-hidden mt-[30px] rounded-[100vmax] button1"><div className="btn1-bg absolute inset-0 -z-[1]"></div>EXPLORE NOW ...</button>
                </div>
            </div>

        </>
    )
}

export default Container3